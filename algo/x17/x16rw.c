...
static __thread char hashOrder[X16RW_HASH_FUNC_COUNT + 1] = { 0 };
...
x16rw_ctx_holder x16rw_ctx __attribute__ ((aligned (64)));

void init_x16rw_ctx()
{
   cubehashInit( &x16rw_ctx.cube, 512, 16, 32 );
};

void x16rw_hash( void* output, const void* input )
{
   uint32_t _ALIGN(128) hash[16];
   x16rw_ctx_holder ctx;
   void *in = (void*) input;
   int size = 80;
   ...
   for ( int i = 8; i < 16; i++ )
   {
      ...
      switch ( algo )
      {
         ...
         break;
         case CUBEHASH:
            memcpy( &ctx.cube, &x16rw_ctx.cube, sizeof(cubehashParam) );
...
// It appears to be working correctly as is.
   if ( s_ntime != pdata[17] )
   {
      ...
      x16_rw_s_getAlgoString( (const char*) (&endiandata[1]), hashOrder );
      s_ntime = ntime;
      if ( opt_debug && !thr_id )
